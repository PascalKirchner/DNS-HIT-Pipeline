...
def get_weight(smiles):
    ...
    weight = descr.ExactMolWt(mol)
    return weight


def get_modifier(smiles):
    modifier = 1
...
def linear(smiles, predictor, invalid_reward=-5.0, threshold=0):
    ...
    modifier = get_modifier(smiles)
    return -1 * prop[0] * modifier - threshold
...
def exponential(smiles, predictor, invalid_reward=-5.0, threshold=0):
    ...
    if len(nan_smiles) == 1:
        return invalid_reward
    modifier = get_modifier(smiles)
    return np.exp(-1 * prop[0] * modifier) - (1 + threshold)
...
def SolvationReward(smiles, predictor, invalid_reward=-5, **kwargs):
    ...
    for i in range(len(scores)):
        if threshold - 0.5 < scores[i] < threshold:
            scores[i] = np.exp(15 * (scores[i] - (threshold - 0.5)))
        elif threshold <= scores[i] < (threshold + 0.5):
            scores[i] = np.exp(-15 * (scores[i] - (threshold + 0.5)))
        else:
            scores[i] = -10
    return scores[0]
...
def QEDReward(smiles, invalid_reward=-5.0, **kwargs):
    ...
    if 'QED' in kwargs:
        ...
        return scores[0]
    for i in range(len(scores)):
        if threshold - 0.05 < scores[i] < threshold:
            scores[i] = np.exp(150 * (scores[i] - (threshold - 0.05)))
        elif threshold <= scores[i] < threshold + 0.05:
            scores[i] = np.exp(-150 * (scores[i] - (threshold + 0.05)))
        else:
            scores[i] = -10
    return scores[0]
...
def LogPReward(smiles, invalid_reward=-5.0, **kwargs):
    ...
    if 'LogP' in kwargs:
        ...
        threshold = 2.5
    for i in range(len(scores)):
        if threshold - 1.5 < scores[i] < threshold:
            scores[i] = np.exp(5 * (scores[i] - (threshold - 1.5)))
        elif threshold <= scores[i] < threshold + 1.5:
            scores[i] = np.exp(-5 * (scores[i] - (threshold + 1.5)))
        else:
            scores[i] = -10
...
def TPSAReward(smiles, invalid_reward=-5.0, **kwargs):
    ...
    if 'TPSA' in kwargs:
        ...
        threshold = 100
    for i in range(len(scores)):
        if threshold - 5 < scores[i] < threshold:
            scores[i] = np.exp(1.5 * (scores[i] - (threshold - 5)))
        elif threshold <= scores[i] < threshold + 5:
            scores[i] = np.exp(-1.5 * (scores[i] - (threshold + 5)))
        else:
            scores[i] = -10
...
class MultiReward:
    def __init__(self, func, use_docking=True, use_logP=True, use_qed=True, use_tpsa=True, use_solvation=True,
                 **kwargs):
        self.func = func
